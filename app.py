...
def quote_builder_page() -> None:
    ...
    st.markdown(
        ...
        This page is designed as the first version of a future multi-product quote basket.
        """
    )
    ...
    if product == "Edge Protector":
        ...
        quote_input_dict = asdict(quote_input)

        product_description = (
            f"Edge Protector {side_1} x {side_2} x {thickness} mm, "
            f"Length {length} mm, {product_type}, {outer_cover}"
        )
        ...
        product_description = (
            f"Tube / Core Ø{diameter} x {thickness} mm, "
            f"Length {length} mm, {product_type}, {outer_cover}"
        )
    ...
    q1.metric(
        "Price / r.m.",
        money(quote_result_dict["price_per_rm_pln"]),
    )

    q2.metric(
        "Price / piece",
        money(quote_result_dict["price_per_piece_pln"]),
    )

    q3.metric(
        "Total value",
        money(quote_result_dict["total_value_pln"]),
    )

    q4.metric(
        "Net weight",
        f"{quote_result_dict['net_weight_kg']:,.2f} kg",
    )
    ...
    summary_df = pd.DataFrame(
        [
            ...
            {
                "Field": "Quantity",
                "Value": quote_input_dict["quantity_pcs"],
            },
            {
